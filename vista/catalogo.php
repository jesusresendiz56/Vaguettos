...
$submarcas = mysqli_query($conn, "SELECT DISTINCT modelo_auto FROM productos ORDER BY modelo_auto");
$fechas = mysqli_query($conn, "SELECT DISTINCT fechas_aplicables FROM productos ORDER BY fechas_aplicables");
$tipos = mysqli_query($conn, "SELECT DISTINCT tipo FROM productos ORDER BY tipo");
...
$where = [];
if ($f_submarca !== '') $where[] = "modelo_auto = '" . mysqli_real_escape_string($conn, $f_submarca) . "'";
if ($f_fecha !== '')    $where[] = "fechas_aplicables = '" . mysqli_real_escape_string($conn, $f_fecha) . "'";
if ($f_tipo !== '')     $where[] = "tipo = '" . mysqli_real_escape_string($conn, $f_tipo) . "'";

$where_sql = count($where) > 0 ? "WHERE " . implode(' AND ', $where) : '';

// Consultar productos filtrados
$productos = mysqli_query($conn, "SELECT * FROM productos $where_sql");
?>
...
<body>
  ...
  <header>
    ...
    <nav>
      ...
      <a href="#" class="nav-link">Catalago de Productos</a>
